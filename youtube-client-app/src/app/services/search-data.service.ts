...
export class SearchDataService {
  ...
  sortResultByDate() {
    ...
    if (firstItemDate > lastItemDate) {
      this.resultData.sort((a, b) => +new Date(a.snippet.publishedAt) - +new Date(b.snippet.publishedAt))
    } else if (firstItemDate < lastItemDate) {
      this.resultData.sort((a, b) => +new Date(b.snippet.publishedAt) - +new Date(a.snippet.publishedAt))
    } else {
      return
    }
    this.dataChanged = Date.now()
  ...
  sortResultByViews() {
    ...
    if (this.resultData[0].statistics.viewCount < this.resultData[lastIndex].statistics.viewCount) {
      this.resultData.sort((a, b) => a.statistics.viewCount - b.statistics.viewCount)
    } else if (this.resultData[0].statistics.viewCount > this.resultData[lastIndex].statistics.viewCount) {
      this.resultData.sort((a, b) => b.statistics.viewCount - a.statistics.viewCount)
    } else {
      return
    }
    this.dataChanged = Date.now()
